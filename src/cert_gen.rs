...
pub fn generate_ca_signed_certificate(config: &CertConfig, ca_cert_pem: &str, ca_key_pem: &str) -> Result<(String, String)> {
    ...
    for rdn in ca_subject.iter() {
        for attribute in rdn.iter() {
            ...
            if *attr_type == x509_parser::oid_registry::OID_X509_COMMON_NAME {
                ...
                if let Ok(value_str) = attr_value.as_str() {
                    ca_distinguished_name.push(rcgen::DnType::OrganizationName, value_str.to_string());
                }
                ...
                if let Ok(value_str) = attr_value.as_str() {
                    ca_distinguished_name.push(rcgen::DnType::StateOrProvinceName, value_str.to_string());
                }
                ...
                if let Ok(value_str) = attr_value.as_str() {
                    ca_distinguished_name.push(rcgen::DnType::OrganizationalUnitName, value_str.to_string());
                }
            }
        }
    }
    ...
    info!("CA-signed certificate generated successfully for {}", config.common_name);
...
fn pem_to_der(pem_data: &str) -> Result<Vec<u8>> {
    ...
    let start_pos = pem_data.find(start_marker)
        .ok_or_else(|| anyhow!("No certificate found in PEM data"))?;
    let end_pos = pem_data.find(end_marker)
    ...
    use base64::prelude::*;
    BASE64_STANDARD.decode(base64_clean)
...
mod tests {
    ...
    // Helper function to run OpenSSL commands for certificate verification
    fn run_openssl_command(args: &[&str], input: Option<&str>) -> Result<String, Box<dyn std::error::Error>> {
        ...
        if output.status.success() {
            ...
            Err(format!("OpenSSL command failed: {}", String::from_utf8_lossy(&output.stderr)).into())
        }
    }
    ...
    fn generate_ca_with_openssl(temp_dir: &std::path::Path, ca_config: &CertConfig) -> Result<(String, String), Box<dyn std::error::Error>> {
...
"#,
        ...
        run_openssl_command(&[
            ...
            "-out", ca_key_path.to_str().unwrap()
        ], None)?;
        
        // Generate self-signed CA certificate
        run_openssl_command(&[
            "req", "-new", "-x509", "-key", ca_key_path.to_str().unwrap(),
            "-out", ca_cert_path.to_str().unwrap(),
            "-days", &ca_config.validity_days.to_string(),
    ...
    fn generate_server_with_openssl(temp_dir: &std::path::Path, server_config: &CertConfig, ca_cert_pem: &str, ca_key_pem: &str) -> Result<(String, String), Box<dyn std::error::Error>> {
...
"#,
        ...
        run_openssl_command(&[
            "ecparam", "-genkey", "-name", "prime256v1",
            "-out", server_key_path.to_str().unwrap()
        ], None)?;
        
        // Generate certificate signing request
        run_openssl_command(&[
            "req", "-new", "-key", server_key_path.to_str().unwrap(),
            "-out", server_csr_path.to_str().unwrap(),
            "-config", server_config_path.to_str().unwrap()
        ], None)?;
        
        // Sign server certificate with CA
        run_openssl_command(&[
            "x509", "-req", "-in", server_csr_path.to_str().unwrap(),
            "-CA", ca_cert_path.to_str().unwrap(),
            "-CAkey", ca_key_path.to_str().unwrap(),
            "-CAcreateserial",
            "-out", server_cert_path.to_str().unwrap(),
            "-days", &server_config.validity_days.to_string(),
            "-extensions", "v3_req",
            "-extfile", server_config_path.to_str().unwrap()
        ], None)?;
    ...
    fn test_generate_server_certificate_with_new_ca_vs_openssl() {
        ...
        let (openssl_ca_cert, openssl_ca_key) = generate_ca_with_openssl(temp_dir.path(), &ca_config)
            .expect("OpenSSL CA generation should succeed");
        let (openssl_server_cert, _openssl_server_key) = generate_server_with_openssl(
            temp_dir.path(), &server_config, &openssl_ca_cert, &openssl_ca_key
        ...
        let openssl_ca_text = run_openssl_command(&[
            "x509", "-in", openssl_ca_cert_path.to_str().unwrap(), "-text", "-noout"
        ], None).expect("OpenSSL CA cert parsing should succeed");
        
        let our_ca_text = run_openssl_command(&[
            "x509", "-in", our_ca_cert_path.to_str().unwrap(), "-text", "-noout"
        ], None).expect("Our CA cert parsing should succeed");
        
        // Verify both CA certificates have CA extensions
        assert!(openssl_ca_text.contains("CA:TRUE"), "OpenSSL CA should have CA constraint");
        assert!(our_ca_text.contains("CA:TRUE"), "Our CA should have CA constraint");
        assert!(openssl_ca_text.contains("Certificate Sign"), "OpenSSL CA should have cert signing usage");
        assert!(our_ca_text.contains("Certificate Sign"), "Our CA should have cert signing usage");
        
        // 2. Compare server certificates
        let openssl_server_text = run_openssl_command(&[
            "x509", "-in", openssl_server_cert_path.to_str().unwrap(), "-text", "-noout"
        ], None).expect("OpenSSL server cert parsing should succeed");
        
        let our_server_text = run_openssl_command(&[
            "x509", "-in", our_server_cert_path.to_str().unwrap(), "-text", "-noout"
        ], None).expect("Our server cert parsing should succeed");
        
        println!("OpenSSL server certificate text:\n{}", openssl_server_text);
        
        // Verify both server certificates have proper subject information
        assert!(openssl_server_text.contains("CN=test.example.com"), "OpenSSL server cert should have correct CN");
    
        // Check that our certificate does have the correct subject (just different format)
        assert!(our_server_text.contains("CN=test.example.com"), "Our server cert should have correct CN");
        ...
        assert!(openssl_server_text.contains("DNS:test.example.com"), "OpenSSL server cert should have DNS SAN");
        assert!(our_server_text.contains("DNS:test.example.com"), "Our server cert should have DNS SAN");
        assert!(openssl_server_text.contains("DNS:alt.example.com"), "OpenSSL server cert should have alt DNS SAN");
        assert!(our_server_text.contains("DNS:alt.example.com"), "Our server cert should have alt DNS SAN");
        assert!(openssl_server_text.contains("IP Address:127.0.0.1"), "OpenSSL server cert should have IP SAN");
        assert!(our_server_text.contains("IP Address:127.0.0.1"), "Our server cert should have IP SAN");
        
        // Verify both have server authentication extensions
        assert!(openssl_server_text.contains("TLS Web Server Authentication"), "OpenSSL server cert should have server auth");
        assert!(our_server_text.contains("TLS Web Server Authentication"), "Our server cert should have server auth");
        
        // 3. Verify certificate chain validation
        let openssl_server_verify = run_openssl_command(&[
            "verify", "-CAfile", openssl_ca_cert_path.to_str().unwrap(), openssl_server_cert_path.to_str().unwrap()
        ], None);
        assert!(openssl_server_verify.is_ok(), "OpenSSL generated certificate chain should be valid");
        assert!(openssl_server_verify.unwrap().contains("OK"), "OpenSSL certificate verification should pass");
        ...
        println!("OpenSSL CA subject: {}", 
            run_openssl_command(&["x509", "-in", openssl_ca_cert_path.to_str().unwrap(), "-subject", "-noout"], None).unwrap_or_default());
        println!("Our CA subject: {}", 
            run_openssl_command(&["x509", "-in", our_ca_cert_path.to_str().unwrap(), "-subject", "-noout"], None).unwrap_or_default());
        println!("Our server issuer: {}", 
            run_openssl_command(&["x509", "-in", our_server_cert_path.to_str().unwrap(), "-issuer", "-noout"], None).unwrap_or_default());
        
        // This will likely fail due to the implementation issue
        let our_server_verify = run_openssl_command(&[
            "verify", "-CAfile", our_ca_cert_path.to_str().unwrap(), our_server_cert_path.to_str().unwrap()
        ], None);
        ...
        if our_server_verify.is_err() {
            ...
            eprintln!("The function creates a simplified CA certificate structure (CN=CA) instead of");
            eprintln!("using the actual CA certificate parameters for signing the server certificate.");
    ...
    fn test_generate_server_certificate_with_existing_ca_vs_openssl() {
        ...
        let (existing_ca_cert_pem, existing_ca_key_pem) = generate_root_ca_certificate(&ca_config)
        ...
        let (openssl_server_cert, _openssl_server_key) = generate_server_with_openssl(
            temp_dir.path(), &server_config, &existing_ca_cert_pem, &existing_ca_key_pem
        ).expect("OpenSSL server certificate generation should succeed");
        ...
        if let Err(ref e) = result {
            println!("Error generating server certificate with existing CA: {}", e);
        }
        
        assert!(result.is_ok(), "Our function with existing CA should succeed");
        ...
        assert!(openssl_server_text.contains("CN=server.example.org"), "OpenSSL server cert should have correct CN");
        assert!(our_server_text.contains("CN=server.example.org"), "Our server cert should have correct CN");
        
        // Verify SANs
        assert!(openssl_server_text.contains("DNS:server.example.org"), "OpenSSL server cert should have primary DNS SAN");
        assert!(our_server_text.contains("DNS:server.example.org"), "Our server cert should have primary DNS SAN");
        assert!(openssl_server_text.contains("DNS:api.example.org"), "OpenSSL server cert should have secondary DNS SAN");
        assert!(our_server_text.contains("DNS:api.example.org"), "Our server cert should have secondary DNS SAN");
        assert!(openssl_server_text.contains("IP Address:192.168.1.100"), "OpenSSL server cert should have IP SAN");
        assert!(our_server_text.contains("IP Address:192.168.1.100"), "Our server cert should have IP SAN");
        
        // Check certificate chain validation
        let openssl_verify = run_openssl_command(&[
            "verify", "-CAfile", ca_cert_path.to_str().unwrap(), openssl_server_cert_path.to_str().unwrap()
        ], None);
        assert!(openssl_verify.is_ok() && openssl_verify.unwrap().contains("OK"), 
        ...
        let our_verify = run_openssl_command(&[
            "verify", "-CAfile", ca_cert_path.to_str().unwrap(), our_server_cert_path.to_str().unwrap()
        ], None);
        
        // Print issuer information for debugging
        println!("Existing CA subject: {}", 
            run_openssl_command(&["x509", "-in", ca_cert_path.to_str().unwrap(), "-subject", "-noout"], None).unwrap_or_default());
        println!("Our server issuer: {}", 
            run_openssl_command(&["x509", "-in", our_server_cert_path.to_str().unwrap(), "-issuer", "-noout"], None).unwrap_or_default());
        
        if our_verify.is_err() {
            eprintln!("IMPLEMENTATION BUG DETECTED:");
            eprintln!("Our certificate does not verify against the existing CA, but OpenSSL's does.");
        ...
        assert!(cert_result.ca_cert_pem.is_some(), "Function should return the existing CA certificate");
        assert_eq!(cert_result.ca_cert_pem.unwrap(), existing_ca_cert_pem, "Returned CA should match existing CA");
        assert!(cert_result.ca_key_pem.is_none(), "Function should not return existing CA private key");
    }
}
