...
impl DynamicCertificateManager {
    ...
    pub fn new(cache_dir: &str, ca_cert_path: &str, ca_key_path: &str) -> Result<Self> {
        ...
        info!("Dynamic certificate manager initialized with cache dir: {}", cache_dir);
    ...
    fn get_from_disk_cache(&self, cache_key: &str) -> Result<Option<CachedCertificate>> {
        ...
        let cert_pem = fs::read_to_string(&cert_path)
            .map_err(|e| anyhow!("Failed to read cached certificate {}: {}", cert_path.display(), e))?;
        let key_pem = fs::read_to_string(&key_path)
            .map_err(|e| anyhow!("Failed to read cached private key {}: {}", key_path.display(), e))?;

        // Get file creation time
        let created_at = cert_path.metadata()
    ...
    fn store_in_disk_cache(&self, cache_key: &str, cert: &CachedCertificate) -> Result<()> {
        ...
        fs::write(&cert_path, &cert.cert_pem)
            .map_err(|e| anyhow!("Failed to write cached certificate {}: {}", cert_path.display(), e))?;
        fs::write(&key_path, &cert.key_pem)
            .map_err(|e| anyhow!("Failed to write cached private key {}: {}", key_path.display(), e))?;
    ...
    fn generate_certificate_for_host(&self, _cache_key: &str, hostname: &str) -> Result<CachedCertificate> {
    ...
    pub fn cleanup_old_certificates(&self, max_age_days: u64) -> Result<()> {
                    ...
                    if let Ok(created) = metadata.created() {
                        if now.duration_since(created).unwrap_or_default() > max_age {
                            if let Err(e) = fs::remove_file(entry.path()) {
                                warn!("Failed to remove old certificate file {:?}: {}", entry.path(), e);
                            } else {
        ...
        info!("Certificate cleanup completed, removed certificates older than {} days", max_age_days);
        Ok(())
    }

    /// Get cache statistics
    pub fn get_cache_stats(&self) -> Result<CacheStats> {
        let memory_count = self.memory_cache.read()
            .map_err(|_| anyhow!("Failed to acquire memory cache read lock"))?
            .len();
        ...
        let disk_count = fs::read_dir(&self.cache_dir)?
            ...
            .count() / 2; // Divide by 2 because we have .cert.pem and .key.pem files
...
mod tests {
    ...
    fn test_normalize_hostname() {
        ...
        assert_eq!(manager.normalize_hostname("192.168.1.1:8080"), "192.168.1.1");
    }
